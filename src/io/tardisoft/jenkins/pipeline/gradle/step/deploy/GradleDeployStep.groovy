package io.tardisoft.jenkins.pipeline.gradle.step.deploy
 ...
 */
class GradleDeployStep extends GradleStep implements Serializable, Deploy {
    /**
    ...
    /**
     * @deprecated Deprecated in favor of the {@link io.tardisoft.jenkins.pipeline.maven.step.deploy.MavenGithubPagesDeployStep} does nothing now, will be removed in future
     */
    @Deprecated
    boolean deploySite
    ...
    @Override
    def run(def script) {
        for (MavenBuildGoal g : gradleGoals) {
    ...
    def deployRelease(def script) {
        ...
        script.withCredentials([script.usernamePassword(credentialsId: gitCredentialsId, usernameVariable: 'GIT_USER', passwordVariable: 'GIT_PASS')]) {
            script.withCredentials([script.usernamePassword(credentialsId: artifactoryCredentialsId, usernameVariable: 'MAVEN_USER', passwordVariable: 'MAVEN_PASS')]) {
                ...
                List<String> args = []
                args.add("publish")
                args.add("-PmavenUser=\${GIT_USER}")
                args.add("-PmavenPassword=\${GIT_PASS}")
